...
#define FIXED_DEFAULT                               \
   0 /* Are elements fixed in place or do they move \
...
enum {
   ANGLE_ROLL = 1000,
   ANGLE_OPPOSITE_ROLL = 1001
}; /* For the roll indicator, do we roll with
      the angle measured or opposite it. */

#define GOOGLE_MAPS_API                                                                   \
   "https://maps.googleapis.com/maps/api/staticmap?center=%f,%f&size=%dx%d&format=png32&" \
 ...
 *
 * FIXME: These are getting a bit out of hand, so I think I need to break these up into their
 * components. After my latest work getting YouTube streaming working... it's worse. Sorry.
 */
...
#define GST_PIPE_INPUT                                                                            \
   "appsrc name=srcEncode ! "                                                                     \
   "video/x-raw, width=(int)%d, height=(int)%d, format=(string)RGBA, framerate=(fraction)%d/1 ! " \
   "queue max-size-buffers=30 ! clocksync ! "

/* === VIDEO PROCESSING COMPONENTS === */
#ifdef PLATFORM_JETSON
#ifdef SOFTWARE_ENCODE
/* Jetson software encoding paths */
#define GST_PIPE_VIDEO_MAIN                  \
   "nvvidconv ! video/x-raw, format=I420 ! " \
   "x264enc bitrate=16000 speed-preset=1 ! "

#define GST_PIPE_VIDEO_STREAM                                               \
   "nvvidconv ! video/x-raw, format=I420, width=(int)%d, height=(int)%d ! " \
   "x264enc bitrate=8000 speed-preset=1 ! "

#define GST_PIPE_VIDEO_HLS                   \
   "nvvidconv ! video/x-raw, format=I420 ! " \
   "x264enc bitrate=8000 tune=zerolatency ! "

#define GST_PIPE_VIDEO_YOUTUBE                                                 \
   "videoconvert ! video/x-raw, width=(int)%d, height=(int)%d, format=I420 ! " \
...
#error \
    "Software encoding is not supported on Raspberry Pi platform. Please undefine SOFTWARE_ENCODE."
#else
...
#define GST_PIPE_VIDEO_STREAM                                                  \
   "videoconvert ! video/x-raw, format=I420, width=(int)%d, height=(int)%d ! " \
...
#define GST_PIPE_QUEUE_VIDEO \
   "queue name=video_queue max-size-buffers=30 max-size-time=0 max-size-bytes=0 ! mux. "
#define GST_PIPE_INPUT_QUEUE_LEAKY                                                                 \
   "queue name=input_queue max-size-buffers=10 max-size-time=0 max-size-bytes=0 leaky=downstream " \
   "! "

/* === COMPLETE PIPELINE DEFINITIONS === */

/* Recording pipeline */
#define GST_ENC_PIPELINE \
   GST_PIPE_INPUT        \
   GST_PIPE_VIDEO_MAIN   \
   GST_PIPE_PARSE        \
   GST_PIPE_QUEUE " " GST_PIPE_AUDIO " " GST_PIPE_MUXER GST_PIPE_FILE_OUT

/* Recording + Streaming pipeline */
#define GST_PIPE_AUDIO_TEE                                                  \
...
#define GST_ENCSTR_PIPELINE                                                        \
   GST_PIPE_INPUT                                                                  \
   "tee name=raw_split ! "                                                         \
   "queue name=record_queue max-size-buffers=10 max-size-time=0 max-size-bytes=0 " \
   "! " GST_PIPE_VIDEO_MAIN GST_PIPE_PARSE "queue ! filemux. "                     \
   "raw_split. ! "                                                                 \
   "queue name=stream_queue max-size-buffers=10 max-size-time=0 max-size-bytes=0 " \
   "leaky=downstream ! " GST_PIPE_VIDEO_YOUTUBE "h264parse config-interval=1 ! "   \
   "queue ! streammux. " GST_PIPE_AUDIO_TEE "matroskamux name=filemux ! "          \
   "filesink location=%s "                                                         \
