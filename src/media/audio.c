...
void *audio_thread(void *arg) {
   ...
   while (!checkShutdown()) {
      ...
      if ((in_size = mq_timedreceive(this_thread->qd_client, in_buffer, MSG_BUFFER_SIZE, NULL,
                                     &queue_timeout)) == -1) {
      ...
      if ((pcm = snd_pcm_open(&pcm_handle, PCM_DEVICE, SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
         printf("[%d] ERROR: Can't open \"%s\" PCM device. %s", this_thread->thread_id, PCM_DEVICE,
                snd_strerror(pcm));
      }
...
#ifdef AUDIO_DEBUG
      LOG_INFO("[%d] Bitstream is %d channel, %ldHz", this_thread->thread_id, vi->channels,
               vi->rate);
      LOG_INFO("[%d] Decoded length: %ld samples", this_thread->thread_id,
               (long)ov_pcm_total(&vf, -1));
      LOG_INFO("[%d] Encoded by: %s", this_thread->thread_id, ov_comment(&vf, -1)->vendor);
#endif
      ...
      if ((pcm = snd_pcm_hw_params_set_access(pcm_handle, params, SND_PCM_ACCESS_RW_INTERLEAVED)) <
          0) {
         LOG_ERROR("[%d] ERROR: Can't set interleaved mode. %s", this_thread->thread_id,
                   snd_strerror(pcm));
      }
      ...
      if ((pcm = snd_pcm_hw_params_set_channels(pcm_handle, params, vi->channels)) < 0) {
         LOG_ERROR("[%d] ERROR: Can't set channels number. %s", this_thread->thread_id,
                   snd_strerror(pcm));
      }
      ...
      if ((pcm = snd_pcm_hw_params(pcm_handle, params)) < 0) {
         LOG_ERROR("[%d] ERROR: Can't set harware parameters. %s", this_thread->thread_id,
                   snd_strerror(pcm));
      }
      ...
      if ((err = snd_pcm_prepare(pcm_handle)) < 0) {
         LOG_ERROR("[%d] Cannot prepare audio interface for use (%s)", this_thread->thread_id,
                   snd_strerror(err));
         exit(1);
      }
      ...
      while (!eof) {
         ...
         if (ret == 0) {
            ...
            if ((pcm = snd_pcm_writei(pcm_handle, buff, ret / vi->channels / 2)) == -EPIPE) {
               ...
               LOG_ERROR("[%d] ERROR. Can't write to PCM device. %s", this_thread->thread_id,
                         snd_strerror(pcm));
            }
         }
      }
